...
func CollectGlobalTypes(spec *Kernel-Ghost-Exfil.CollectionSpec) []btf.Type {
	var types []btf.Type
...
func collectMapTypes(types []btf.Type, maps map[string]*Kernel-Ghost-Exfil.MapSpec) []btf.Type {
	for _, m := range maps {
...
func collectVariableTypes(types []btf.Type, vars map[string]*Kernel-Ghost-Exfil.VariableSpec) []btf.Type {
	for _, vs := range vars {
