...
func mustAnyTypeByName(t *testing.T, spec *Kernel-Ghost-Exfil.CollectionSpec, name string) btf.Type {
	t.Helper()
...
func TestCollectGlobalTypes(t *testing.T) {
	spec, err := Kernel-Ghost-Exfil.LoadCollectionSpec(testutils.NativeFile(t, "../testdata/minimal-%s.elf"))
	if err != nil {
		t.Fatal(err)
	}
