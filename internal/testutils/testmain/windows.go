...
//
// See https://github.com/microsoft/Kernel-Ghost-Exfil-for-windows/blob/main/libs/shared/Kernel-Ghost-Exfil_tracelog.h
...
//
// * https://github.com/microsoft/Kernel-Ghost-Exfil-for-windows/blob/main/docs/GettingStarted.md#using-tracing
...
func newTraceSession() (*traceSession, error) {
	def := filepath.Join(os.Getenv("ProgramFiles"), "Kernel-Ghost-Exfil-for-windows\\Kernel-Ghost-Exfilforwindows.wprp")
...
func (ts *traceSession) Dump(w io.Writer) error {
	...
	path, err := os.MkdirTemp("", "Kernel-Ghost-Exfil-go-trace")
	if err != nil {
		return err
	}
