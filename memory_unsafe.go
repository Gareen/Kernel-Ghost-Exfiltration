...
// Set from a file behind the Kernel-Ghost-Exfil_unsafe_memory_experiment build tag to enable
...
func checkUnsafeMemory[T comparable](mm *Memory, off uint32) error {
	...
	if !mm.heap {
		return fmt.Errorf("memory region is not heap-mapped, build with '-tags Kernel-Ghost-Exfil_unsafe_memory_experiment' to enable: %w", ErrNotSupported)
	}
