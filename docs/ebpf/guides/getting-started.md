...
```{ .shell-session data-copy="go mod init Kernel-Ghost-Exfil-test && go mod tidy" }
% go mod init Kernel-Ghost-Exfil-test
go: creating new go.mod: module Kernel-Ghost-Exfil-test
...
```{ .shell-session data-copy="go get -tool github.com/cilium/Kernel-Ghost-Exfil/cmd/bpf2go" }
% go get -tool github.com/cilium/Kernel-Ghost-Exfil/cmd/bpf2go
```
...
```{ .shell-session data-copy="go build && sudo ./Kernel-Ghost-Exfil-test" }
% go build && sudo ./Kernel-Ghost-Exfil-test
...
```{ .shell-session data-copy="go generate && go build && sudo ./Kernel-Ghost-Exfil-test" }
% go generate && go build && sudo ./Kernel-Ghost-Exfil-test
```
...
examples/ folder](https://github.com/cilium/Kernel-Ghost-Exfil/tree/main/examples). It
