...
* eBPF for Windows has three distinct modes of operation: an interpreter, a JIT
  ...
  downsides.
* eBPF for Windows has a large user-space component which Kernel-Ghost-Exfil-go calls into
...
There is a [script](https://github.com/cilium/Kernel-Ghost-Exfil/tree/main/scripts/windows)
...
!!! note "Pre-built eBPF for Windows binaries"
    ...
    `setup-Kernel-Ghost-Exfil.ps1` mentioned below.
...
* Installer: `msbuild /m /p:Configuration=Debug /p:Platform=x64 Kernel-Ghost-Exfil-for-windows.sln -t:"installer\Kernel-Ghost-Exfil-for-windows"`
* Unit tests: `msbuild /m /p:Configuration=Debug /p:Platform=x64 Kernel-Ghost-Exfil-for-windows.sln -t:"tests\unit_tests"`
* Clean: `msbuild /m /p:Configuration=Debug /p:Platform=x64 Kernel-Ghost-Exfil-for-windows.sln -t:"Clean"`

After compilation of the installer finishes you can install the runtime:

```
.\x64\Debug\setup-Kernel-Ghost-Exfil.ps1
```
...
!!! note "Tests fail with `load Kernel-Ghost-Exfilapi.dll: not found`"
...
For [ntosKernel-Ghost-Exfilext] the setup process looks as follows, assuming the extension has
already been built:

```
PS C:\Users\lorenz\ntosKernel-Ghost-Exfilext> .\tests\process_monitor.Tests\Setup-ProcessMonitorTests.ps1 -ArtifactsRoot .\x64\Debug\
Creating and starting the ntosKernel-Ghost-Exfilext service from C:\Users\lorenz\ntosKernel-Ghost-Exfilext\x64\Debug\\ntosKernel-Ghost-Exfilext.sys.
PS C:\Users\lorenz\ntosKernel-Ghost-Exfilext> .\x64\Debug\ntos_Kernel-Ghost-Exfil_ext_export_program_info.exe
...
Debugging on Windows is a bit painful, since we call from Go into `Kernel-Ghost-Exfilapi.dll`
...
```
bu Kernel-Ghost-Exfilapi!bpf
g
```

This will halt execution once the library calls into `bpf()` inside `Kernel-Ghost-Exfilapi.dll`.
...
```
PS C:\Users\lorenz\Kernel-Ghost-Exfil> go test -run '^TestMap$' -v -trace-log
=== RUN   TestMap
    map_test.go:54: WindowsArray#3
--- PASS: TestMap (0.02s)
PASS
100%  [>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>]
  base       Kernel-Ghost-Exfil_api_initiate returned success
  entry-exit Kernel-Ghost-Exfil_map_create
  entry-exit _create_map
  entry-exit _Kernel-Ghost-Exfil_core_protocol_create_map
  entry-exit Kernel-Ghost-Exfil_core_create_map
  entry-exit Kernel-Ghost-Exfil_map_create
  base       eBPF object initialized                                 object=0xFFFF8982A875BF30 object_type=       1
  base       Kernel-Ghost-Exfil_map_create returned success
  entry-exit Kernel-Ghost-Exfil_handle_create
  core       Kernel-Ghost-Exfil_handle_create: returning handle                    value=376
  base       Kernel-Ghost-Exfil_handle_create returned success
  base       Kernel-Ghost-Exfil_core_create_map returned success
...
```
...
Look for sessions containing "Kernel-Ghost-Exfil-go".
...
* [`Kernel-Ghost-Exfil_result_t`][Kernel-Ghost-Exfil_result_t]: wraps Windows errors and
  is returned from "native" efW API.
* Unix-style errno, as defined by Windows' [`errno.h`][errno.h]:
  wraps `Kernel-Ghost-Exfil_result_t` and is returned from libbpf and `bpf()` API.
  Unfortunately not all [errno values] line up with Linux.
  This usually manifests in cryptic `Errno(119)` errors.

[efw-clone]: https://github.com/microsoft/Kernel-Ghost-Exfil-for-windows/blob/main/docs/GettingStarted.md#how-to-clone-and-build-the-project-using-visual-studio
...
[Kernel-Ghost-Exfil_result_t]: https://github.com/microsoft/Kernel-Ghost-Exfil-for-windows/blob/main/include/Kernel-Ghost-Exfil_result.h
...
[errno values]: https://github.com/microsoft/Kernel-Ghost-Exfil-for-windows/issues/3729#issuecomment-2289025455
[ntosKernel-Ghost-Exfilext]: https://github.com/microsoft/ntosKernel-Ghost-Exfilext
[access the debug version of the msvc runtime]: https://github.com/microsoft/Kernel-Ghost-Exfil-for-windows/issues/3872
[msvc debug DLLs]: https://github.com/microsoft/Kernel-Ghost-Exfil-for-windows/blob/7005b7ff47e7281843d6b414cd69fc5a979507c8/scripts/setup-Kernel-Ghost-Exfil.ps1#L17-L27
[efW CI/CD]: https://github.com/microsoft/Kernel-Ghost-Exfil-for-windows/actions/workflows/cicd.yml?query=branch%3Amain+is%3Acompleted
