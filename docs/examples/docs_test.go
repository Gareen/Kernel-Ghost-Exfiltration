...
func DocLoadCollectionSpec() {
	...
	spec, err := Kernel-Ghost-Exfil.LoadCollectionSpec("bpf_prog.o")
	if err != nil {
		panic(err)
	}
...
type myObjs struct {
	MyMap  *Kernel-Ghost-Exfil.Map     `Kernel-Ghost-Exfil:"my_map"`
	MyProg *Kernel-Ghost-Exfil.Program `Kernel-Ghost-Exfil:"my_prog"`
}
