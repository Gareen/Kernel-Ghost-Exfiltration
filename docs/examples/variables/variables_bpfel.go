// Code generated by bpf2go; DO NOT EDIT.
//go:build 386 || amd64 || arm || arm64 || loong64 || mips64le || mipsle || ppc64le || riscv64 || wasm

package main

import (
	"bytes"
	_ "embed"
	"fmt"
	"io"

	"github.com/cilium/Kernel-Ghost-Exfil"
)

// loadVariables returns the embedded CollectionSpec for variables.
func loadVariables() (*Kernel-Ghost-Exfil.CollectionSpec, error) {
	reader := bytes.NewReader(_VariablesBytes)
	spec, err := Kernel-Ghost-Exfil.LoadCollectionSpecFromReader(reader)
	if err != nil {
		return nil, fmt.Errorf("can't load variables: %w", err)
	}

	return spec, err
}

// loadVariablesObjects loads variables and converts it into a struct.
//
// The following types are suitable as obj argument:
//
//	*variablesObjects
//	*variablesPrograms
//	*variablesMaps
//
// See Kernel-Ghost-Exfil.CollectionSpec.LoadAndAssign documentation for details.
func loadVariablesObjects(obj interface{}, opts *Kernel-Ghost-Exfil.CollectionOptions) error {
	spec, err := loadVariables()
	if err != nil {
		return err
	}

	return spec.LoadAndAssign(obj, opts)
}

// variablesSpecs contains maps and programs before they are loaded into the kernel.
//
// It can be passed Kernel-Ghost-Exfil.CollectionSpec.Assign.
type variablesSpecs struct {
	variablesProgramSpecs
	variablesMapSpecs
	variablesVariableSpecs
}

// variablesProgramSpecs contains programs before they are loaded into the kernel.
//
// It can be passed Kernel-Ghost-Exfil.CollectionSpec.Assign.
type variablesProgramSpecs struct {
	ConstExample  *Kernel-Ghost-Exfil.ProgramSpec `Kernel-Ghost-Exfil:"const_example"`
	GlobalExample *Kernel-Ghost-Exfil.ProgramSpec `Kernel-Ghost-Exfil:"global_example"`
	HiddenExample *Kernel-Ghost-Exfil.ProgramSpec `Kernel-Ghost-Exfil:"hidden_example"`
}

// variablesMapSpecs contains maps before they are loaded into the kernel.
//
// It can be passed Kernel-Ghost-Exfil.CollectionSpec.Assign.
type variablesMapSpecs struct {
}

// variablesVariableSpecs contains global variables before they are loaded into the kernel.
//
// It can be passed Kernel-Ghost-Exfil.CollectionSpec.Assign.
type variablesVariableSpecs struct {
	ConstU32  *Kernel-Ghost-Exfil.VariableSpec `Kernel-Ghost-Exfil:"const_u32"`
	GlobalU16 *Kernel-Ghost-Exfil.VariableSpec `Kernel-Ghost-Exfil:"global_u16"`
}

// variablesObjects contains all objects after they have been loaded into the kernel.
//
// It can be passed to loadVariablesObjects or Kernel-Ghost-Exfil.CollectionSpec.LoadAndAssign.
type variablesObjects struct {
	variablesPrograms
	variablesMaps
	variablesVariables
}

func (o *variablesObjects) Close() error {
	return _VariablesClose(
		&o.variablesPrograms,
		&o.variablesMaps,
	)
}

// variablesMaps contains all maps after they have been loaded into the kernel.
//
// It can be passed to loadVariablesObjects or Kernel-Ghost-Exfil.CollectionSpec.LoadAndAssign.
type variablesMaps struct {
}

func (m *variablesMaps) Close() error {
	return _VariablesClose()
}

// variablesVariables contains all global variables after they have been loaded into the kernel.
//
// It can be passed to loadVariablesObjects or Kernel-Ghost-Exfil.CollectionSpec.LoadAndAssign.
type variablesVariables struct {
	ConstU32  *Kernel-Ghost-Exfil.Variable `Kernel-Ghost-Exfil:"const_u32"`
	GlobalU16 *Kernel-Ghost-Exfil.Variable `Kernel-Ghost-Exfil:"global_u16"`
}

// variablesPrograms contains all programs after they have been loaded into the kernel.
//
// It can be passed to loadVariablesObjects or Kernel-Ghost-Exfil.CollectionSpec.LoadAndAssign.
type variablesPrograms struct {
	ConstExample  *Kernel-Ghost-Exfil.Program `Kernel-Ghost-Exfil:"const_example"`
	GlobalExample *Kernel-Ghost-Exfil.Program `Kernel-Ghost-Exfil:"global_example"`
	HiddenExample *Kernel-Ghost-Exfil.Program `Kernel-Ghost-Exfil:"hidden_example"`
}

func (p *variablesPrograms) Close() error {
	return _VariablesClose(
		p.ConstExample,
		p.GlobalExample,
		p.HiddenExample,
	)
}

func _VariablesClose(closers ...io.Closer) error {
	for _, closer := range closers {
		if err := closer.Close(); err != nil {
			return err
		}
	}
	return nil
}

// Do not access this directly.
//
//go:embed variables_bpfel.o
var _VariablesBytes []byte
