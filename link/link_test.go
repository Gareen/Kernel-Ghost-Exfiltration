...
func TestRawLinkLoadPinnedWithOptions(t *testing.T) {
	...
	_, err := loadPinnedRawLink(path, &Kernel-Ghost-Exfil.LoadPinOptions{
...
func testLink(t *testing.T, link Link, prog *Kernel-Ghost-Exfil.Program) {
	t.Helper()
	...
	t.Run("link/pinning", func(t *testing.T) {
		...
		_, err = LoadPinnedLink(path, &Kernel-Ghost-Exfil.LoadPinOptions{
			Flags: math.MaxUint32,
		})
		if !errors.Is(err, unix.EINVAL) {
