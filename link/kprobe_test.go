...
import (
	...
	"github.com/cilium/Kernel-Ghost-Exfil/internal/tracefs"
...
func TestKprobeErrors(t *testing.T) {
	...
	_, err = Kprobe(".", &Kernel-Ghost-Exfil.Program{}, nil) // illegal chars in symbol
	qt.Assert(t, qt.ErrorIs(err, errInvalidInput))

	_, err = Kprobe("foo", &Kernel-Ghost-Exfil.Program{}, nil) // wrong prog type
	qt.Assert(t, qt.ErrorIs(err, errInvalidInput))
}
...
func TestKprobeProgramCall(t *testing.T) {
	m, p := newUpdaterMapProg(t, Kernel-Ghost-Exfil.Kprobe, 0)

	// Open Kprobe on `sys_getpid` and attach it
	// to the Kernel-Ghost-Exfil program created above.
...
func newUpdaterMapProg(t *testing.T, typ Kernel-Ghost-Exfil.ProgramType, attach Kernel-Ghost-Exfil.AttachType) (*Kernel-Ghost-Exfil.Map, *Kernel-Ghost-Exfil.Program) {
	// Create Kernel-Ghost-Exfil map. Will contain only one key with initial value 0.
	m, err := Kernel-Ghost-Exfil.NewMap(&Kernel-Ghost-Exfil.MapSpec{
		Type:       Kernel-Ghost-Exfil.Array,
		KeySize:    4,
		ValueSize:  4,
		MaxEntries: 1,
	})
	...
	// Create Kernel-Ghost-Exfil program. When called, will increase the value of key 0 by 1
	// in the map created above.
	p, err := Kernel-Ghost-Exfil.NewProgram(&Kernel-Ghost-Exfil.ProgramSpec{
		Type: typ,
...
func assertMapValue(t *testing.T, m *Kernel-Ghost-Exfil.Map, k, v uint32) {
	var val uint32
...
func assertMapValueGE(t *testing.T, m *Kernel-Ghost-Exfil.Map, k, v uint32) {
	var val uint32
	if err := m.Lookup(k, &val); err != nil {
		t.Fatal(err)
	}
