...
func newRawLink(t *testing.T) (*RawLink, *Kernel-Ghost-Exfil.Program) {
	prog, err := Kernel-Ghost-Exfil.NewProgram(&Kernel-Ghost-Exfil.ProgramSpec{
		Type: Kernel-Ghost-Exfil.WindowsBind,
	...
	link, err := AttachRawLink(RawLinkOptions{
		Program: prog,
		Attach:  Kernel-Ghost-Exfil.AttachWindowsBind,
	})
	qt.Assert(t, qt.IsNil(err))
...
func TestProcessLink(t *testing.T) {
	array, err := Kernel-Ghost-Exfil.NewMap(&Kernel-Ghost-Exfil.MapSpec{
		Type:       Kernel-Ghost-Exfil.WindowsArray,
		Name:       "process_state",
		KeySize:    4,
		ValueSize:  4,
		MaxEntries: 1,
