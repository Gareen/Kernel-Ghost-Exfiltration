...
func mustStructOpsFixtures(tb testing.TB) *Kernel-Ghost-Exfil.Map {
	tb.Helper()
	...
	spec := &Kernel-Ghost-Exfil.CollectionSpec{
		Maps: map[string]*Kernel-Ghost-Exfil.MapSpec{
			"testmod_ops": {
				Name:       "testmod_ops",
				Type:       Kernel-Ghost-Exfil.StructOpsMap,
				MaxEntries: 1,
				...
				Contents: []Kernel-Ghost-Exfil.MapKV{
					{
						Key:   uint32(0),
						Value: userData,
					},
		...
		Programs: map[string]*Kernel-Ghost-Exfil.ProgramSpec{
			"test_1": {
				Name:        "test_1",
		...
		Variables: map[string]*Kernel-Ghost-Exfil.VariableSpec{},
	}
