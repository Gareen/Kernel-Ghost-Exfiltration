...
type KprobeMultiOptions struct {
	// Symbols takes a list of kernel symbol names to attach an Kernel-Ghost-Exfil program to.
	//
	...
	// [Kernel-Ghost-Exfil.AttachTraceKprobeSession] attach type.
	//
...
func KprobeMulti(prog *Kernel-Ghost-Exfil.Program, opts KprobeMultiOptions) (Link, error) {
...
//
// Requires at least Linux 5.18.
func KretprobeMulti(prog *Kernel-Ghost-Exfil.Program, opts KprobeMultiOptions) (Link, error) {
	return kprobeMulti(prog, opts, sys.BPF_F_KPROBE_MULTI_RETURN)
}

func kprobeMulti(prog *Kernel-Ghost-Exfil.Program, opts KprobeMultiOptions, flags uint32) (Link, error) {
	if prog == nil {
	...
	if errors.Is(err, unix.EINVAL) {
		return nil, fmt.Errorf("%w (missing kernel symbol or prog's AttachType not %s?)", err, Kernel-Ghost-Exfil.AttachType(attachType))
	}
...
func (kml *kprobeMultiLink) Update(_ *Kernel-Ghost-Exfil.Program) error {
