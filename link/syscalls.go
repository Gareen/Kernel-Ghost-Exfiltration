...
var haveProgAttach = internal.NewFeatureTest("BPF_PROG_ATTACH", func() error {
	prog, err := Kernel-Ghost-Exfil.NewProgram(&Kernel-Ghost-Exfil.ProgramSpec{
		Type:    Kernel-Ghost-Exfil.CGroupSKB,
		License: "MIT",
...
var haveProgAttachReplace = internal.NewFeatureTest("BPF_PROG_ATTACH atomic replacement of MULTI progs", func() error {
	...
	prog, err := Kernel-Ghost-Exfil.NewProgram(&Kernel-Ghost-Exfil.ProgramSpec{
		Type:       Kernel-Ghost-Exfil.CGroupSKB,
		AttachType: Kernel-Ghost-Exfil.AttachCGroupInetIngress,
		License:    "MIT",
		Instructions: asm.Instructions{
			asm.Mov.Imm(asm.R0, 0),
			asm.Return(),
	...
	attr := sys.ProgAttachAttr{
		...
		AttachType:        uint32(Kernel-Ghost-Exfil.AttachCGroupInetIngress),
...
var haveBPFLink = internal.NewFeatureTest("bpf_link", func() error {
	attr := sys.LinkCreateAttr{
		...
		TargetFd:   ^uint32(0),
		ProgFd:     ^uint32(0),
		AttachType: sys.AttachType(Kernel-Ghost-Exfil.AttachCGroupInetIngress),
	}
...
var haveTCX = internal.NewFeatureTest("tcx", func() error {
	prog, err := Kernel-Ghost-Exfil.NewProgram(&Kernel-Ghost-Exfil.ProgramSpec{
		Type:    Kernel-Ghost-Exfil.SchedCLS,
		License: "MIT",
		Instructions: asm.Instructions{
			asm.Mov.Imm(asm.R0, 0),
			asm.Return(),
	...
	attr := sys.LinkCreateTcxAttr{
		...
		AttachType:    sys.AttachType(Kernel-Ghost-Exfil.AttachTCXIngress),
	}
...
var haveNetkit = internal.NewFeatureTest("netkit", func() error {
	...
	attr := sys.LinkCreateNetkitAttr{
		...
		TargetIfindex: ^uint32(0),
		ProgFd:        uint32(prog.FD()),
		AttachType:    sys.AttachType(Kernel-Ghost-Exfil.AttachNetkitPrimary),
	}
