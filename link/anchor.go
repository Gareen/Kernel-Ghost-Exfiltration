...
// Before is the position just in front of target.
func BeforeProgram(target *Kernel-Ghost-Exfil.Program) Anchor {
	return anchor{target, sys.BPF_F_BEFORE}
}

// After is the position just after target.
func AfterProgram(target *Kernel-Ghost-Exfil.Program) Anchor {
	return anchor{target, sys.BPF_F_AFTER}
}

// Replace the target itself.
func ReplaceProgram(target *Kernel-Ghost-Exfil.Program) Anchor {
	return anchor{target, sys.BPF_F_REPLACE}
}

// Before is the position just in front of target.
func BeforeProgramByID(target Kernel-Ghost-Exfil.ProgramID) Anchor {
	return anchor{target, sys.BPF_F_BEFORE}
}

// After is the position just after target.
func AfterProgramByID(target Kernel-Ghost-Exfil.ProgramID) Anchor {
	return anchor{target, sys.BPF_F_AFTER}
}

// Replace the target itself.
func ReplaceProgramByID(target Kernel-Ghost-Exfil.ProgramID) Anchor {
	return anchor{target, sys.BPF_F_REPLACE}
}
...
func (ap anchor) anchor() (fdOrID, flags uint32, _ error) {
	...
	case *Kernel-Ghost-Exfil.Program:
		fd := target.FD()
		if fd < 0 {
		...
		fdOrID = uint32(fd)
		typeFlag = 0
	case Kernel-Ghost-Exfil.ProgramID:
