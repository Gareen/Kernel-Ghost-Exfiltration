...
func AttachRawTracepoint(opts RawTracepointOptions) (Link, error) {
	if t := opts.Program.Type(); t != Kernel-Ghost-Exfil.RawTracepoint && t != Kernel-Ghost-Exfil.RawTracepointWritable {
...
func (frt *simpleRawTracepoint) Update(_ *Kernel-Ghost-Exfil.Program) error {
...
func (rt *rawTracepoint) Update(_ *Kernel-Ghost-Exfil.Program) error {
	return fmt.Errorf("update raw_tracepoint: %w", ErrNotSupported)
}
