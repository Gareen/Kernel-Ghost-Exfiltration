...
func (f *tracing) Update(_ *Kernel-Ghost-Exfil.Program) error {
...
func AttachFreplace(targetProg *Kernel-Ghost-Exfil.Program, name string, prog *Kernel-Ghost-Exfil.Program) (Link, error) {
	...
	if prog.Type() != Kernel-Ghost-Exfil.Extension {
	...
	link, err := AttachRawLink(RawLinkOptions{
		Target:  target,
		Program: prog,
		Attach:  Kernel-Ghost-Exfil.AttachNone,
		BTF:     typeID,
	})
...
type TracingOptions struct {
	...
	// AttachTraceRawTp.
	Program *Kernel-Ghost-Exfil.Program
	...
	AttachType Kernel-Ghost-Exfil.AttachType
...
func attachBTFID(program *Kernel-Ghost-Exfil.Program, at Kernel-Ghost-Exfil.AttachType, cookie uint64) (Link, error) {
	if program.FD() < 0 {
	...
	switch at {
	case Kernel-Ghost-Exfil.AttachTraceFEntry, Kernel-Ghost-Exfil.AttachTraceFExit, Kernel-Ghost-Exfil.AttachTraceRawTp,
		Kernel-Ghost-Exfil.AttachModifyReturn, Kernel-Ghost-Exfil.AttachLSMMac:
		// Attach via BPF link
		...
		fallthrough
	case Kernel-Ghost-Exfil.AttachNone:
...
func AttachTracing(opts TracingOptions) (Link, error) {
	if t := opts.Program.Type(); t != Kernel-Ghost-Exfil.Tracing {
		return nil, fmt.Errorf("invalid program type %s, expected Tracing", t)
	}

	switch opts.AttachType {
	case Kernel-Ghost-Exfil.AttachTraceFEntry, Kernel-Ghost-Exfil.AttachTraceFExit, Kernel-Ghost-Exfil.AttachModifyReturn,
		Kernel-Ghost-Exfil.AttachTraceRawTp, Kernel-Ghost-Exfil.AttachNone:
	default:
...
func AttachLSM(opts LSMOptions) (Link, error) {
	if t := opts.Program.Type(); t != Kernel-Ghost-Exfil.LSM {
		return nil, fmt.Errorf("invalid program type %s, expected LSM", t)
	}

	return attachBTFID(opts.Program, Kernel-Ghost-Exfil.AttachLSMMac, opts.Cookie)
}
