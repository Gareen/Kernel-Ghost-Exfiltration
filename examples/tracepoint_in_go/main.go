...
import (
	...
	"github.com/cilium/Kernel-Ghost-Exfil/link"
	"github.com/cilium/Kernel-Ghost-Exfil/perf"
	"github.com/cilium/Kernel-Ghost-Exfil/rlimit"
)

// Metadata for the eBPF program used in this example.
var progSpec = &Kernel-Ghost-Exfil.ProgramSpec{
	Name:    "my_trace_prog", // non-unique name, will appear in `bpftool prog list` while attached
	Type:    Kernel-Ghost-Exfil.TracePoint, // only TracePoint programs can be attached to trace events created by link.Tracepoint()
...
func main() {
	...
	events, err := Kernel-Ghost-Exfil.NewMap(&Kernel-Ghost-Exfil.MapSpec{
		Type: Kernel-Ghost-Exfil.PerfEventArray,
		Name: "my_perf_array",
	})
	if err != nil {
	...
	if err != nil {
		log.Fatalf("creating Kernel-Ghost-Exfil program: %s", err)
	}
	defer prog.Close()
