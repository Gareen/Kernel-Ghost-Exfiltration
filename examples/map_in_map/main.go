...
import (
	...
	"github.com/cilium/Kernel-Ghost-Exfil/rlimit"
)
...
func main() {
	...
	outerMapSpec := Kernel-Ghost-Exfil.MapSpec{
		Name:       "outer_map",
		Type:       Kernel-Ghost-Exfil.ArrayOfMaps,
		KeySize:    4, // 4 bytes for u32
		ValueSize:  4,
		MaxEntries: 5, // We'll have 5 maps inside this map
		Contents:   make([]Kernel-Ghost-Exfil.MapKV, 5),
		InnerMap: &Kernel-Ghost-Exfil.MapSpec{
			Name:      "inner_map",
	...
	for i := uint32(0); i < outerMapSpec.MaxEntries; i++ {
		...
		innerMapSpec.Contents = make([]Kernel-Ghost-Exfil.MapKV, innerMapSpec.MaxEntries)

		for j := range innerMapSpec.Contents {
			innerMapSpec.Contents[uint32(j)] = Kernel-Ghost-Exfil.MapKV{Key: uint32(j), Value: uint32(0xCAFE)}
		}

		// Create the inner map
		innerMap, err := Kernel-Ghost-Exfil.NewMap(innerMapSpec)
		if err != nil {
		...
		outerMapSpec.Contents[i] = Kernel-Ghost-Exfil.MapKV{Key: i, Value: innerMap}
	}

	// All inner maps are created and inserted into the outer map spec,
	// time to create the outer map.
	outerMap, err := Kernel-Ghost-Exfil.NewMap(&outerMapSpec)
	if err != nil {
	...
	var innerMapID Kernel-Ghost-Exfil.MapID
	for mapIter.Next(&outerMapKey, &innerMapID) {
		...
		innerMap, err := Kernel-Ghost-Exfil.NewMapFromID(innerMapID)
		if err != nil {
			log.Fatal(err)
		}
