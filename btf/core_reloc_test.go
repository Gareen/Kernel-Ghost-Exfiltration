...
func TestCORERelocationLoad(t *testing.T) {
	...
	spec, err := Kernel-Ghost-Exfil.LoadCollectionSpecFromReader(fh)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, progSpec := range spec.Programs {
		t.Run(progSpec.Name, func(t *testing.T) {
			...
			prog, err := Kernel-Ghost-Exfil.NewProgramWithOptions(progSpec, Kernel-Ghost-Exfil.ProgramOptions{
...
func TestCORERelocationRead(t *testing.T) {
	...
	tests := []struct {
		name string
		opts Kernel-Ghost-Exfil.ProgramOptions
	}{
		{
			name: "KernelTypes",
			opts: Kernel-Ghost-Exfil.ProgramOptions{
	...
	for _, progSpec := range spec.Programs {
		for _, test := range tests {
			t.Run(progSpec.Name+"_"+test.name, func(t *testing.T) {
				prog, err := Kernel-Ghost-Exfil.NewProgramWithOptions(progSpec, test.opts)
				testutils.SkipIfNotSupported(t, err)
				if err != nil {
					t.Fatal("Load program:", err)
				}
...
func TestLD64IMMReloc(t *testing.T) {
	...
	coll, err := Kernel-Ghost-Exfil.NewCollection(spec)
...
func TestCOREPoisonLineInfo(t *testing.T) {
	spec, err := Kernel-Ghost-Exfil.LoadCollectionSpec(testutils.NativeFile(t, "../testdata/errors-%s.elf"))
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := Kernel-Ghost-Exfil.NewProgramWithOptions(progSpec, Kernel-Ghost-Exfil.ProgramOptions{
				KernelTypes: empty,
			})
			testutils.SkipIfNotSupported(t, err)

			var ve *Kernel-Ghost-Exfil.VerifierError
