...
import (
	...
	"github.com/cilium/Kernel-Ghost-Exfil/internal/kallsyms"
	"github.com/cilium/Kernel-Ghost-Exfil/internal/kconfig"
...
// has an `Kernel-Ghost-Exfil` tag and its type is *ProgramSpec, *MapSpec or *VariableSpec.
// The tag's value specifies the name of the program or map as
// found in the CollectionSpec.
//
//	struct {
//	    Foo     *Kernel-Ghost-Exfil.ProgramSpec  `Kernel-Ghost-Exfil:"xdp_foo"`
//	    Bar     *Kernel-Ghost-Exfil.MapSpec      `Kernel-Ghost-Exfil:"bar_map"`
//	    Var     *Kernel-Ghost-Exfil.VariableSpec `Kernel-Ghost-Exfil:"some_var"`
//	    Ignored int
//	}
//
...
// a Program or Map if it has an `Kernel-Ghost-Exfil` tag and its type is *Program or *Map.
...
//
//	struct {
//	    Foo     *Kernel-Ghost-Exfil.Program `Kernel-Ghost-Exfil:"xdp_foo"`
//	    Bar     *Kernel-Ghost-Exfil.Map     `Kernel-Ghost-Exfil:"bar_map"`
//	    Ignored int
//	}
//
// opts may be nil.
...
// has an `Kernel-Ghost-Exfil` tag and its type is *Program or *Map.
// The tag's value specifies the name of the program or map as
// found in the CollectionSpec.
//
//	struct {
...
// structField represents a struct field containing the Kernel-Ghost-Exfil struct tag.
type structField struct {
	reflect.StructField
	value reflect.Value
}

// Kernel-Ghost-ExfilFields extracts field names tagged with 'Kernel-Ghost-Exfil' from a struct type.
// Keep track of visited types to avoid infinite recursion.
func Kernel-Ghost-ExfilFields(structVal reflect.Value, visited map[reflect.Type]bool) ([]structField, error) {
	if visited == nil {
	...
	for i := 0; i < structType.NumField(); i++ {
		...
		name := field.Tag.Get("Kernel-Ghost-Exfil")
		if name != "" {
			fields = append(fields, field)
			continue
		}

		// If the field does not have an Kernel-Ghost-Exfil tag, but is a struct or a pointer
		...
		inner, err := Kernel-Ghost-ExfilFields(v, visited)
		if err != nil {
...
// assignValues attempts to populate all fields of 'to' tagged with 'Kernel-Ghost-Exfil'.
//
...
func assignValues(to interface{},
	...
	fields, err := Kernel-Ghost-ExfilFields(toValue.Elem(), nil)
	if err != nil {
		return err
	}
	...
	for _, field := range fields {
		...
		tag := field.Tag.Get("Kernel-Ghost-Exfil")
		if strings.Contains(tag, ",") {
			return fmt.Errorf("field %s: Kernel-Ghost-Exfil tag contains a comma", field.Name)
		}
