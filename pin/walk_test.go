...
func TestWalkDir(t *testing.T) {
	...
	qt.Assert(t, qt.Equals(reflect.TypeOf(pin.Object), reflect.TypeFor[*Kernel-Ghost-Exfil.Map]()))
	qt.Assert(t, qt.Equals(pin.Path, mapPath))

	pin, err, ok = next()
	qt.Assert(t, qt.IsTrue(ok))
	qt.Assert(t, qt.IsNil(err))
	qt.Assert(t, qt.Equals(reflect.TypeOf(pin.Object), reflect.TypeFor[*Kernel-Ghost-Exfil.Program]()))
