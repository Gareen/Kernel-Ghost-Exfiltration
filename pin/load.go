...
// of [Kernel-Ghost-Exfil.Map], [Kernel-Ghost-Exfil.Program], or [link.Link].
//
// Trying to open anything other than a bpf object is an error.
func Load(path string, opts *Kernel-Ghost-Exfil.LoadPinOptions) (Pinner, error) {
	...
	case sys.BPF_TYPE_MAP:
		return Kernel-Ghost-Exfil.NewMapFromFD(fd.Disown())
	case sys.BPF_TYPE_PROG:
		return Kernel-Ghost-Exfil.NewProgramFromFD(fd.Disown())
