...
func mustOutputSamplesProg(tb testing.TB, sampleMessages ...sampleMessage) (*Kernel-Ghost-Exfil.Program, *Kernel-Ghost-Exfil.Map) {
	tb.Helper()

	events, err := Kernel-Ghost-Exfil.NewMap(&Kernel-Ghost-Exfil.MapSpec{
		Type:       Kernel-Ghost-Exfil.WindowsRingBuf,
		MaxEntries: 4096,
	})
	...
	prog, err := Kernel-Ghost-Exfil.NewProgram(&Kernel-Ghost-Exfil.ProgramSpec{
		License:      "MIT",
		Type:         Kernel-Ghost-Exfil.WindowsSample,
		Instructions: insns,
	})
	qt.Assert(tb, qt.IsNil(err))
	tb.Cleanup(func() {
