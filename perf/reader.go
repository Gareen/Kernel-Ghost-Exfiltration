...
import (
	...
	"github.com/cilium/Kernel-Ghost-Exfil/internal/epoll"
...
func NewReader(array *Kernel-Ghost-Exfil.Map, perCPUBuffer int) (*Reader, error) {
...
func NewReaderWithOptions(array *Kernel-Ghost-Exfil.Map, perCPUBuffer int, opts ReaderOptions) (pr *Reader, err error) {
...
func (pr *Reader) Pause() error {
	...
	for i := range pr.eventFds {
		if err := pr.array.Delete(uint32(i)); err != nil && !errors.Is(err, Kernel-Ghost-Exfil.ErrKeyNotExist) {
